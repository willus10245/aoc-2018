const arrayOfSplitWords = `bazvmqthjtrnlosiecxyghkwud
...
const arrayOfSortedLetters = arrayOfSplitWords.map(array => {
  const arrayCopy = [...array]
  return arrayCopy.sort()
})

const twiceCount = arrayOfSortedLetters.filter(array => {
  let found = false
  for (let i = 0; i < array.length; i++) {
    if (array[i] === array[i + 1] && array[i] !== array[i + 2] && array[i] !== array[i - 1]) {
      found = true
    }
  }
  return found
...
const thriceCount = arrayOfSortedLetters.filter(array => {
  let found = false
  for (let i = 0; i < array.length; i++) {
...
const result1 = twiceCount * thriceCount

const getNumberOfDifferences = (first, second) => first.filter((val, i) => val !== second[i]).length

const almostMatching = arrayOfSplitWords.filter((arr1, _, arrs) => {
  let hasAlmostMatch = false
  for (let i = 0; i < arrs.length; i++) {
    if (getNumberOfDifferences(arr1, arrs[i]) === 1) {
      hasAlmostMatch = true
    }
  }
  return hasAlmostMatch
})

const result2 = almostMatching[0].filter((val, i) => val === almostMatching[1][i]).join('')
