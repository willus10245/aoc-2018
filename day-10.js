const input = `position=<-30052,  -9918> velocity=< 3,  1>
...
position=< 20344,  20343> velocity=<-2, -2>`
  .split('\n')
  .map(line => line.split(/<|>/))
  .map(arr => [arr[1], arr[3]].map(entry => entry.split(', ').map(Number)))
  .map(point => ({
    coords: point[0],
    velocity: point[1]
  }))
...
const maxX = Math.max(...input.map(obj => obj.coords).map(arr => arr[0]))
const minX = Math.min(...input.map(obj => obj.coords).map(arr => arr[0]))
const maxY = Math.max(...input.map(obj => obj.coords).map(arr => arr[1]))
const minY = Math.min(...input.map(obj => obj.coords).map(arr => arr[1]))
console.log(maxX, minX)
console.log(maxY, minY)

function calcNextPosition(coords, velocity, t) {
  const x = coords[0] + velocity[0] * t
  const y = coords[1] + velocity[1] * t
  return [x, y]
}

function makeGrid(points, t) {
  const grid = Array(1000).fill(Array(1000).fill(null)).map(arr => arr.map(_ => '.'))
  const pointsAtTimeT = points.map(point => calcNextPosition(point.coords, point.velocity, t))

  for (point of pointsAtTimeT) {
    const [x, y] = point
    if (grid[x] && grid[y]) {
      grid[y] = grid[y]
        .map((char, i) => {
...
// function anyIncludes(grid, string) {
//   const linesContaining = grid.filter(line => line.includes(string))
//   return linesContaining.length > 0
// }

// let timeElapsed = 0
// const interval = setInterval(function() {
  fs.writeFileSync('day-10.txt', '')
//   // fs.appendFileSync('day-10.txt', `Time Elapsed: ${timeElapsed}`)
//   console.log(`Time elapsed: ${timeElapsed}`)
//   const grid = makeGrid(input, timeElapsed)

//   const lines = grid.map(line => line.join('').concat('\n'))

//   // for (let i = 0; i < lines.length; i++) {
//   //   fs.appendFileSync('day-10.txt', lines[i])
//   // }

//   if (anyIncludes(lines, 'XXXX')) {
//     clearInterval(interval)
//   }
//   timeElapsed++
// }, 0)

const grid = makeGrid(input, 10086)
const lines = grid.map(line => line.join('').concat('\n'))
