const input = `[1518-06-02 23:58] Guard #179 begins shift
...
let currentGuard
let lastFellAsleep
const createSleepChart = () => new Array(60).fill(0)

for (entry of input) {
  const [timestamp, note] = entry
  if (note.includes('#')) {
    const guard = note.match(/\d{3,4}/)[0]
    currentGuard = guard
  }
  if (note.includes('falls')) {
    lastFellAsleep = timestamp.substring(timestamp.length - 2)
  }
  if (note.includes('wakes')) {
    currTime = timestamp.substring(timestamp.length - 2)
    // const sleepTime = currTime - lastFellAsleep
    // if (totals[currentGuard]) {
    //   totals[currentGuard] += sleepTime
    // } else {
    //   totals[currentGuard] = sleepTime
    // }
    if (!totals[currentGuard]) {
      totals[currentGuard] = createSleepChart()
    }
    const sleepChart = totals[currentGuard]
    for (let i = Number(lastFellAsleep); i < Number(currTime); i++){
      sleepChart[i]++
    }
  }
}

const longestSleep = Object.values(totals)
  .sort((a, b) => b - a)[0]

const sleepiest = Object.keys(totals).filter(key => totals[key] === longestSleep)

const maxes = Object.keys(totals)
  .reduce((acc, key) => {
    acc[key] = Math.max(...totals[key])
    return acc
  }, {})
...
console.log(totals['1783'].indexOf(17))